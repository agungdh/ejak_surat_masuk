...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Data Seminar</h3>
        </div>
        <div class="card-body">
            <a href="/seminar/create">
                <button class="btn btn-success mb-3"><i class="fa fa-plus"></i> Tambah Seminar</button>
            </a>
            <div class="row mb-4">
                @role('admin')
...
@push('scripts')
    <script>
            ...
            $('#tabel').DataTable({
                ...
                columns: [
                    ...
                    { data: 'nama', name: 'p.nama' },
                        @endrole                    { data: 'materi_pengembangan', name: 's.materi_pengembangan' },
                    { data: 'tanggal_pelaksanaan', name: 's.tanggal_pelaksanaan' },
                    { data: 'jumlah_jam', name: 's.jumlah_jam' },
                    { data: 'berkas', name: 's.filename' },
        ...
        async function hapusData(id) {
            ...
            if (result.value) {
                try {
                    await axios.delete(`/seminar/${id}`);

                    $('#tabel').DataTable().ajax.reload();

                    toastr.success('Seminar berhasil dihapus');
                } catch (e) {
